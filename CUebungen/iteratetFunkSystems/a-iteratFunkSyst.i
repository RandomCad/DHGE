...
# 29 "./iteratFunkSyst.c" 2

point calcPoints[1000000];

int main(int argc, const char *argv[]){
  srand(time(
# 33 "./iteratFunkSyst.c" 3 4
            ((void *)0)
# 33 "./iteratFunkSyst.c"
                ));
  sdlInit();
  ...
  calcPoints[0]=(point) {0,0,0,0,0};

  for(int i=1;i<1000000 -1;i++){
    int randNum=rand()%100, index=0;
    for(;index<4;index++) if(prozent[index]<randNum) break;
    calcPoints[i]=(point) {
    ...
    if(i>=1){
      ...
      if(calcPoints[i].x>yMax){
        yMax=calcPoints[i].x;
        continue;
      }
      if(calcPoints[i].x<yMin){
        yMin=calcPoints[i].x;
        continue;
      }
    }
  ...
  for(int i=1;i<1000000;i++){
    sdlDrawPoint(((calcPoints[i].x-xMax)/(xMax-xMin))*(800 -1),
                 ((calcPoints[i].y-yMax)/(yMax-yMin))*(600 -1),0,252,0);
  }
